...
void mqtt_client_task(void *pvParameters)
{
    ...
    if (pdPASS != xTaskCreate(btn_task, "Button Task", configMINIMAL_STACK_SIZE, NULL, 2, NULL))
	{
		printf("Failed to create Button task!\n");
		goto exit_cleanup;
	}
    if (pdPASS != xTaskCreate(thermistor_task, "Thermistor Task", 1024, NULL, 2, NULL))
	{
		printf("Failed to create Thermistor task!\n");
		goto exit_cleanup;
	}
